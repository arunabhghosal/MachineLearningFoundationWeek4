...
{\color{incolor}In [{\color{incolor}2}]:} \PY{c+c1}{\PYZsh{} Import libraries necessary for this project}
        ...
        \PY{c+c1}{\PYZsh{} print(\PYZdq{}Boston housing dataset has \PYZob{}\PYZcb{} data points with \PYZob{}\PYZcb{} variables each.\PYZdq{}, format(str(*data.shape)))}
\end{Verbatim}


    \begin{Verbatim}[commandchars=\\\{\}]
/home/arunabhghosal/anaconda3/lib/python3.6/site-packages/sklearn/cross\_validation.py:41: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the refactored classes and functions are moved. Also note that the interface of the new CV iterators are different from that of this module. This module will be removed in 0.20.
  "This module will be removed in 0.20.", DeprecationWarning)
/home/arunabhghosal/anaconda3/lib/python3.6/site-packages/sklearn/learning\_curve.py:22: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the functions are moved. This module will be removed in 0.20
  DeprecationWarning)
...
{\color{incolor}In [{\color{incolor}3}]:} \PY{c+c1}{\PYZsh{} TODO: Minimum price of the data}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Minimum price: \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{minimum\PYZus{}price}\PY{p}{)}\PY{p}{)}
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Maximum price: \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{maximum\PYZus{}price}\PY{p}{)}\PY{p}{)}
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Mean price: \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{mean\PYZus{}price}\PY{p}{)}\PY{p}{)}
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Median price \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{median\PYZus{}price}\PY{p}{)}\PY{p}{)}
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Standard deviation of prices: \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{std\PYZus{}price}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
...
For a higher RM, one would expect to observe a higher MEDV. This is
because more rooms would imply more space, thereby costing more, taking
all other factors constant.

LSTAT

For a higher LSTAT, one would expect to observe a lower MEDV. This is
because people tend to pay more cost to get place in rich locality.

PTRATIO

For a higher LSTAT, one would expect to observe a lower MEDV. This is
because there would be a lower teacher-to-student ratio resulting in
less attention dedicated to each student that may impair their
...
{\color{incolor}In [{\color{incolor}4}]:} \PY{c+c1}{\PYZsh{} Calculate the performance of this model}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Model has a coefficient of determination, R\PYZca{}2, of }\PY{l+s+si}{\PYZob{}:.3f\PYZcb{}}\PY{l+s+s1}{.}\PY{l+s+s1}{\PYZsq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{score}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
...
R2 value is 0.923 which is close to 1 therefore this model have
succefully captured the variation of the target variable.
...
We shouldn't train and test on the same data. This is becuase maximizing
training accuracy rewards overly complex models that overfit the
training data.

We can split the dataset so that the model can be trained and tested on
different dataset.
...
max\_depth = 3 Testing score increases with training points. It reaches
aproximately to 0.8. 0.8 is a high score. Training score decreases
slightly with training points. It reaches aproximately to 0.8. It fit
dataset well. The model fits and generalizes well. This model may not
need anymore points.
...
With max depth as 1 the model sufferes from high bias. We can conclude
this because both the training score and validation score is less
(Underfit).

With max depth as 10 the model suffers from high variance. We can
conclude this because even though training score is good, validation
score is bad (Overfit).
...
Maximum depth of 4 best describes the unseen data. Both the training and
validation scores are good and there is no gap between the two. After
depth 4 the validation score is going low.
...
Grid search technique allows to choose a model based on grid of
parameters. It tries every possible combination of value of provided
parameters to select best model.

It tries to optimize hyperparameter.
...
In k-fold cross-validation dataset is split into K bucktes of equal
size. Each of the buckets is used as validation set 1 time, and acts as
the training set K-1 times. Then the average performance is used to
estimate the performance.

Without k-fold cross-validation may select combination of hyper
parameter value which overfit but actualy performs poorly.
...
{\color{incolor}In [{\color{incolor}8}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}make\PYZus{}scorer\PYZsq{}, \PYZsq{}DecisionTreeRegressor\PYZsq{}, and \PYZsq{}GridSearchCV\PYZsq{}}
        ...
        \PY{c+c1}{\PYZsh{}     params = dict(max\PYZus{}depth=(1,11))}
            
            \PY{n}{dt\PYZus{}range} \PY{o}{=} \PY{n+nb}{range}\PY{p}{(}\PY{l+m+mi}{1}\PY{p}{,} \PY{l+m+mi}{11}\PY{p}{)}
        \PY{c+c1}{\PYZsh{}     params = dict(max\PYZus{}depth=dt\PYZus{}range)}
            ...
            \PY{n}{grid} \PY{o}{=} \PY{n}{GridSearchCV}\PY{p}{(}\PY{n}{regressor}\PY{p}{,} \PY{n}{params}\PY{p}{,} \PY{n}{cv}\PY{o}{=}\PY{n}{cv\PYZus{}sets}\PY{p}{,} \PY{n}{scoring}\PY{o}{=}\PY{n}{scoring\PYZus{}fnc}\PY{p}{)}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/home/arunabhghosal/anaconda3/lib/python3.6/site-packages/sklearn/grid\_search.py:42: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the refactored classes and functions are moved. This module will be removed in 0.20.
  DeprecationWarning)
...
4 My guess at question 6 is same as this answer
...
Minimum price: 105,000.00 Maximum price: 1,024,800.00 Mean price:
454,342.94 Median price 438,900.00 Standard deviation of prices:
165,340.28

Predicted Values:

Predicted selling price for Client 1's home: 406,933.33 Predicted
selling price for Client 2's home: 232,200.00 Predicted selling price
for Client 3's home: 938,053.85

For client 1: Price is in the range of mean(454,342.94) and
median(438,900.00). It is reasonable.

For client 2: Price is below the range of mean and median. It is also
reasonable because number of rooms are less than client 1, Neighborhood
poverty level is high, Student-teacher ratio of nearby schools is also
high.

For client 3: Price is nearly equivalent that of maximum price. This is
also reasonable as it has many rooms(double of client 2), Neighborhood
poverty level is low, Student-teacher ratio of nearby schools is also
low.
...
Only three features are present in dataset which is not sufficient at
all because number of rooms can't define the size of the home. Data
collected from a rural city may not be applicable as the demographics
would change.

Therefor constructed model should not be used in real world settings.
